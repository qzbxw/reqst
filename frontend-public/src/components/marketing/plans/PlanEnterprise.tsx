...
export function PlanEnterprise() {
  const { language } = useUI();
  const text = PUBLIC_PLAN_COPY[language];
  const marketing = PUBLIC_MARKETING_COPY[language];
  const product = text.enterprise;
  const seoText = language === "en" ? planSEOEn.enterprise : planSEORu.enterprise;
  ...
  return (
    <MarketingLayout language={language}>
      ...
      {/* Hero Section */}
      <header className="lend-hero lend-hero--centered lend-ent-hero" ref={reveal}>
