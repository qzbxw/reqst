...
export function PlanDeveloper() {
  const { language } = useUI();
  const text = PUBLIC_PLAN_COPY[language];
  const marketing = PUBLIC_MARKETING_COPY[language];
  const product = text.developer;
  const seoText = language === "en" ? planSEOEn.developer : planSEORu.developer;
  ...
  return (
    <MarketingLayout language={language}>
      ...
      {/* Hero Section */}
      <header className="lend-hero lend-hero--centered lend-dev-hero" ref={reveal}>
