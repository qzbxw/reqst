...
export function PlanMerchant() {
  const { language } = useUI();
  const text = PUBLIC_PLAN_COPY[language];
  const marketing = PUBLIC_MARKETING_COPY[language];
  const product = text.merchant;
  const seoText = language === "en" ? planSEOEn.merchant : planSEORu.merchant;
  ...
  return (
    <MarketingLayout language={language}>
      ...
      {/* Hero Section */}
      <header className="lend-hero lend-hero--centered lend-merchant-hero" ref={reveal}>
